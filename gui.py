...
import temper  # Add this line to import the temper module
import os
import subprocess
...
class EnhancedAutoDataLoggerGUI:
    ...
    def read_temperature(self):
        return self.temper.get_temperature()
    ...
    def on_temp_port_selected(self, event):
        ...
        print(f"Selected Temperature Sensor Port: {selected_port}")
        self.setup_temp_sensor(selected_port)
    ...
    def setup_temp_sensor(self, port):
        if self.check_and_request_permissions(port):
            try:
                self.temper = temper.Temper(port)
                print(f"Connected to Temperature Sensor on port: {port}")
                self.temp_sensor_connected = True
                self.temp_status_label.config(text="Connected", foreground="green")
            except Exception as e:
                print(f"Error connecting to Temperature Sensor: {e}")
                messagebox.showerror("Connection Error", f"Error connecting to Temperature Sensor: {e}")
                self.temp_sensor_connected = False
