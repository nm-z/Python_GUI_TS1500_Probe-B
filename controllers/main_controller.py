...
class MainController(QObject):
    ...
    hardware_event = pyqtSignal(dict)  # Add hardware event signal
        ...
        try:
            ...
            for i, angle in enumerate(angles):
                if not self.test_running or self.test_paused:
                    gui_logger.info("Test interrupted")
                    break
                ...
                # Wait for oil to settle, but check for interruption
                gui_logger.info(f"Waiting {parameters['oil_level_time']} seconds for oil stabilization...")
                start_wait = time.time()
                while time.time() - start_wait < parameters['oil_level_time']:
                    if not self.test_running or self.test_paused:
                        gui_logger.info("Test interrupted during wait")
                        break
    ...
    def poll_connections(self):
        ...
        arduino_connected = self.hardware.is_connected()
        
        # Update connection states
        self.connection_states = {
            'arduino': arduino_connected,
            'vna': self.connection_states.get('vna', False),
            'tilt': arduino_connected,  # Tilt is part of Arduino
            'temp': arduino_connected   # Temperature sensor is part of Arduino
        }
    ...
    def connect_hardware(self):
        ...
        try:
            ...
            if self.hardware.connect():
                # Update connection states immediately after successful connection
                self.connection_states['arduino'] = True
    ...
    def load_last_config(self):
        ...
        try:
            config = self.config.load_last_config()
            return config
        except Exception as e:
            raise e
